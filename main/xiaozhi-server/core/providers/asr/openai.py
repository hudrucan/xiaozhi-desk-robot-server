...
class ASRProvider(ASRProviderBase):
    ...
    async def speech_to_text(self, opus_data: List[bytes], session_id: str, artifacts=None) -> Tuple[Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Send model options as multipart form fields.
            data = {
                "model": self.model
            }
            ...
            with open(file_path, "rb") as audio_file:
                ...
                logger.bind(tag=TAG).debug(
                    f"ASR completed in {time.time() - start_time:.3f}s | response: {response.text}"
                )
            ...
            if response.status_code == 200:
                ...
                raise Exception(f"ASR API request failed: {response.status_code} - {response.text}")
