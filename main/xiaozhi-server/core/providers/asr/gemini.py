...
class ASRProvider(ASRProviderBase):
    ...
    async def open_audio_channels(self, conn):
        ...
        try:
            ...
            logger.bind(tag=TAG).warning(
                f"Gemini ASR will connect on first speech: {error}"
            )
    ...
    async def _ensure_session(self, refresh_if_old=True, log_connected=True):
        async with self._connect_lock:
            ...
            if log_connected:
                logger.bind(tag=TAG).info("Gemini ASR session connected")
            return self._session
        ...
        while not self._closed:
            try:
                ...
                logger.bind(tag=TAG).error(
                    f"Gemini ASR audio send failed: {error}"
                )
            ...
            if not self._closed:
                ...
                if idle_close:
                    logger.bind(tag=TAG).debug(
                        "Gemini ASR idle session closed"
                    )
                else:
                    logger.bind(tag=TAG).error(
    ...
    async def _reconnect_after_idle_close(self):
        try:
            ...
            logger.bind(tag=TAG).info(
                "Gemini ASR session reconnected after idle timeout"
            )
        except asyncio.CancelledError:
            pass
        except Exception as error:
            if not self._closed:
                logger.bind(tag=TAG).warning(
                    f"Gemini ASR idle reconnect failed: {error}"
                )
        finally:
            self._reconnect_task = None
    ...
    async def speech_to_text(
        ...
        raise RuntimeError("Gemini ASR only supports streaming audio")
