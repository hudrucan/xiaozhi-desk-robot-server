        ...
        async with AsyncExitStack() as stack:
            try:
                ...
                # 建立SSEClient
                    ...
                    # Preserve the legacy token field while guiding users to headers.
                    if "API_ACCESS_TOKEN" in self.config:
                        headers["Authorization"] = f"Bearer {self.config['API_ACCESS_TOKEN']}"
                        self.logger.bind(tag=TAG).warning(
                            "API_ACCESS_TOKEN is deprecated; set the token directly "
                            "in .mcp_server_settings.json headers, for example "
